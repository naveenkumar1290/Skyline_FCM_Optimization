...
public class CameraUtils {
    ...
    public static File getOutputMediaFile(int type) {
        ...
        File mediaStorageDir = new File(
                ...
                Utility.GALLERY_DIRECTORY_NAME);

        // Create the storage directory if it does not exist
        if (!mediaStorageDir.exists()) {
            if (!mediaStorageDir.mkdirs()) {
                Log.e( Utility.GALLERY_DIRECTORY_NAME, "Oops! Failed create "
                        +  Utility.GALLERY_DIRECTORY_NAME + " directory");
                return null;
            }
        }
        ...
        if (type ==  Utility.MEDIA_TYPE_IMAGE) {
            mediaFile = new File(mediaStorageDir.getPath() + File.separator
                    + "IMG_" + timeStamp + "." +  Utility.IMAGE_EXTENSION);
        } else if (type ==  Utility.MEDIA_TYPE_VIDEO) {
            mediaFile = new File(mediaStorageDir.getPath() + File.separator
                    + "VID_" + timeStamp + "." +  Utility.VIDEO_EXTENSION);
        } else {
            return null;
        }
